...
# PRC 2026 causality experiments (E-C1 alignment-ladder downstream F1,
# E-C3 calibration-buffer composition, E-C4 CORAL regulariser sweep).
# E-C2 contributes no tests: it is a descriptive robustness check on one
# rung's regulariser and makes no paired comparison. Generated by
# stats_causality_fdr.py.
causality_path = OUT / "causality_stats_fdr.csv"
if causality_path.exists():
    caus = pd.read_csv(causality_path)
    for _, r in caus.iterrows():
        rows.append(dict(family="PRC 2026 causality (E-C1, E-C3, E-C4)", comparison=r['comparison'],
                         delta=float(r['delta_pp']), d=float(r['cohens_d']), p=float(r['p'])))
