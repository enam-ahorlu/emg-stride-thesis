# run_classical_featsel_globnorm_loso.py
"""
Orchestrator: runs classical LOSO (SVM + RF) with feature selection across
four conditions (rfe_36, rfe_27, mi_36, mi_27) using GLOBAL z-score
normalization and freq-72 features.

This is the independent-baseline version — same feature selection experiments
but with global norm instead of per_subject, so the improvement from feature
selection can be measured independently from the normalization improvement.

Usage
-----
    python run_classical_featsel_globnorm_loso.py
    python run_classical_featsel_globnorm_loso.py --skip-conditions rfe_27,mi_27
    python run_classical_featsel_globnorm_loso.py --models SVM
"""
...
def run_condition(
    ...
    out_dir = ROOT / f"results_loso_freq_{condition_name}_globnorm"
    cmd = [
    ...
    print(f"[run] models={models}  norm=global  out={out_dir}")
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Orchestrate classical LOSO with feature selection (freq-72, GLOBAL norm)."
    )
    ap.add_argument(
        "--skip-conditions",
        default="",
    ...
    print(f"{'='*70}")
    print("DONE — feature selection ablation (global norm) complete.")
